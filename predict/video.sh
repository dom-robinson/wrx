...
function apt_proj {
    ...
    filename=$(basename "$file" .wav) # filename used to extract variables for the map
    ...
    tle=/home/pi/weather/predict/weather.tle
    ...
    # map to overlay
    /usr/local/bin/wxmap -c g:dark-cyan -c C:light-green -a -T "$satellite" -H $tle -p 0 -l 1 -o $start ${file}-map.png &> wxtoimg.log
    cat wxtoimg.log
    
    # IR for projection:
    /usr/local/bin/wxtoimg -m ${file}-map.png -b -c -e histeq -H const -y full -o ${file}_res.wav $file-IR-proj.png &> proj.log
	...
	/usr/local/bin/wxproj -p mercator -k "%d/%m/%Y - %H:%M UTC" -Q 100 -b 55,30,-5,25 -A 50% -N -v -o $file-IR-proj.png $file-proj.png &> wxproj.log
